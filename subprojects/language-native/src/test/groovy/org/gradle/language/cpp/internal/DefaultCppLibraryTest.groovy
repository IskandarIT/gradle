...
class DefaultCppLibraryTest extends Specification {
    ...
    def setup() {
        library = new DefaultCppLibrary("main", project.objects, project.fileOperations, project.configurations, project.services.get(TargetMachineFactory.class), CollectionCallbackActionDecorator.NOOP)
    }
    ...
    def "uses component name to determine header directories"() {
        ...
        def c1 = new DefaultCppLibrary("a", project.objects, project.fileOperations, project.configurations, project.services.get(TargetMachineFactory.class), CollectionCallbackActionDecorator.NOOP)
        def c2 = new DefaultCppLibrary("b", project.objects, project.fileOperations, project.configurations, project.services.get(TargetMachineFactory.class), CollectionCallbackActionDecorator.NOOP)
